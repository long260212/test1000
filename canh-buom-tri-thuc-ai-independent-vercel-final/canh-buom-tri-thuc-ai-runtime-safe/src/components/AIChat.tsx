...
import { getClientFallbackReply } from '../localAiFallback';
...
export default function AIChat({ lessons, initialLessonId, onNavigateToLesson, currentUser }: AIChatProps) {
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    try {
      ...
      const saved = localStorage.getItem(`physilearn_ai_chat_v2${suffix}`);
      if (saved) {
...
Hãy trình bày bằng tiếng Việt, ngôn ngữ truyền cảm hứng học tập và chuyên nghiệp nhất.`;
  ...
  useEffect(() => {
    ...
    try {
      ...
      localStorage.setItem(`physilearn_ai_chat_v2${suffix}`, JSON.stringify(messages));
    } catch (err) {
  ...
  const handleSendMessage = async (msgContent: string) => {
    ...
    try {
      ...
      console.error("AI endpoint unavailable; browser fallback used:", err);
      setIsUsingGemini(false);
      const botMsg: Message = {
        id: `msg-${Date.now()}-bot-local`,
        role: 'assistant',
        content: getClientFallbackReply(msgContent),
        timestamp: new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })
      };
      setMessages(prev => [...prev, botMsg]);
      setErrorText('');
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full ${
              isUsingGemini 
                ...
                : 'bg-teal-50 dark:bg-teal-950/40 text-teal-700 dark:text-teal-300 border border-teal-100 dark:border-teal-900/30'
            }`}>
              {isUsingGemini ? '🤖 Trí tuệ nhân tạo Gemini' : '✨ Trợ lý học tập'}
            </span>
